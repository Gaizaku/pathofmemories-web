...
export function GuildWarPublication({language,eventId,publicationId}:{language:"th"|"en";eventId:string;publicationId:string}){
 ...
 const roundHeading=(startsAt:string,warType:string)=>{
  ...
  const day=new Intl.DateTimeFormat(th?"th-TH":"en-GB",{timeZone:"Asia/Bangkok",day:"numeric",month:"short",year:"numeric"}).format(value);
  const number=({"19:30":1,"20:00":2,"20:30":3,"21:00":4}[hour+":"+minute]||1);
  return th?`รอบ ${number} (${warType}) · ${hour}.${minute} · ${day}`:`Round ${number} (${warType}) · ${hour}:${minute} · ${day}`;
 };
